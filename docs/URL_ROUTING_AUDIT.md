...
| `OrderApp.tsx` | Card init → hosted `checkoutUrl` (fail-closed if missing). Return query `paid=1` / `payment_error=1` / `payment_pending=1` (+ `sale`). Paid done screen → `/track?token=`. | External checkout URL must be trusted (payment provider). Pending is not treated as paid. |

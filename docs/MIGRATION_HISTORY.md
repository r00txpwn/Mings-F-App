...
- `20260424160000_payment_reconciliation_log.sql` — **`payment_reconciliation_log`**: append-only audit columns for future payment reconciliation (`sale_id`, `provider`, `reconcile_trigger`, `candidate_reason`, `action`, `before_snapshot` / `after_snapshot`, `provider_response`, `error_message`); RLS on, no anon policies yet (service role / future RPC).

...
- `20260804151700_admin_mutation_idempotency.sql` — **money write safety**: `admin_mutation_idempotency` (service-role response cache) + partial unique `client_request_id` on `supplier_account_payments`, `liability_payments`, `bank_withdrawals`, `cash_movements`, `account_transfers`, `salary_payments`. Redeploy **`admin-api`** and staff frontend so clients send `idempotency_key`.

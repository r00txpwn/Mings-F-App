...
function collectEdgeFunctionSignatures() {
  ...
  return runCommand(`find ${fnDir} -name "index.ts" -not -path "*/_shared/*"`)
    .split('\n')
    .filter(Boolean)
    .map((f) => {
...
5. **Logic Issues** — from reading the source: anything logically wrong, risky, or inconsistent with documentation
...
8. **History** — append a one-line summary to the history table (keep existing rows from current QA_STATUS.md)

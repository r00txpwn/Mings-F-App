...
function OrderContent() {
  ...
  const { user, session, loading: authLoading, signInWithGoogle, sendPhoneOtp, verifyPhoneOtp, signOut } =
    useAuth();
  ...
  const [cart, setCart] = useState<CartItem[]>(() => {
    try {
      const saved = localStorage.getItem('mings_cart');
  ...
  useEffect(() => {
    localStorage.setItem('mings_cart', JSON.stringify(cart));
  }, [cart]);
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      setResult(data);
      setCart([]);
      localStorage.removeItem('mings_cart');
      setFlow('done');
      void reloadOrders();
    } catch (e) {
  ...
  const accountPanelT = useMemo(
    () => ({
      orderSignInWithGoogle: t.orderSignInWithGoogle,
      orderOrDivider: t.orderOrDivider,
    ...
    return (
      <div className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
        <div className="mx-auto max-w-md space-y-4">
          ...
          {/* Post-order Google sign-in CTA for guests */}
          {!user && (
            <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-5">
              <p className="mb-3 text-sm font-medium text-slate-200">{t.orderSaveForNextTime}</p>
              <button
                type="button"
                onClick={() => void signInWithGoogle()}
                className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/15 bg-white px-4 py-3 text-sm font-semibold text-slate-800 shadow-sm transition hover:bg-slate-50"
              >
    ...
    <div className="neon-shell flex min-h-screen flex-col pb-[env(safe-area-inset-bottom)]">
      ...
      {flow === 'checkout' && (
        <div className="flex flex-1 flex-col gap-4 p-4 pb-12">
          ...
          {/* Inline Google sign-in nudge for guests */}
          {!user && (
            <div className="neon-card flex flex-col gap-3 p-4">
              <p className="text-sm font-medium text-slate-200">{t.orderFasterCheckoutHint}</p>
              <button
                type="button"
                onClick={() => void signInWithGoogle()}
                className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/15 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 shadow-sm transition hover:bg-slate-50"
              >
              ...
              <p className="text-center text-xs text-slate-500">{t.orderContinueAsGuest}</p>
            </div>
          )}

          {/* Contact details — hidden when signed in (pre-filled from profile) */}
          {user && profile ? (
              ...
              {(
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 rounded-xl border px-2 py-3 text-center transition ${
                    paymentMethod === value
                      ? 'border-cockpit-500 bg-cockpit-500/10 text-white'

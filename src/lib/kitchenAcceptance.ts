 ...
 * Keep in sync with `supabase/functions/_shared/kitchenAcceptance.ts`.
 */
...
/** Subset of `OnlineSettingsRow` used by acceptance logic. */
...
export interface KitchenStatusResult {
  ...
  /** Next calendar opening instant from `hours_json` (strictly after `when` when currently closed; after session end when open). */
  nextOpenAt?: Date;
...
/** Add whole calendar days to a Baku (y,mon,d) — Baku has no DST so +86400000 is safe. */
...
/** First opening instant strictly after `after` (Baku `hours_json`), or null if none in horizon. */
...
/**
 * Next "open boundary" for staff presets: if currently inside hours, the opening after today's
